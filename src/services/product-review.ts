...
class ProductReviewService extends TransactionBaseService {
  ...
  public async stats(filter: { product_id: string[] }): Promise<ProductReviewStats[]> {
    ...
    // Performance optimization: Use Map for O(1) lookups instead of filter
    const resultsByProduct = new Map<string, typeof parsedResults>();
    for (const result of parsedResults) {
      if (!resultsByProduct.has(result.product_id)) {
        resultsByProduct.set(result.product_id, []);
      }
      resultsByProduct.get(result.product_id)!.push(result);
    }

    return filter.product_id.map((product_id) => {
      const filteredResults = resultsByProduct.get(product_id) || [];
      ...
      // Bug fix: Prevent NaN when count is 0
      const average = count > 0 ? parseFloat((totalRatings / count).toFixed(2)) : 0;

      // Performance optimization: Pre-build rating map for O(1) lookups
      const ratingMap = new Map(filteredResults.map(r => [r.rating, r.count]));
      
      const by_rating = Array.from({ length: 6 }, (_, index) => ({
        count: ratingMap.get(index) || 0,
        rating: index,
      }));
